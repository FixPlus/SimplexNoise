 ...
 */
static double grad(int64_t hash, double x) {
    const int64_t h = hash & 0x0F;  // Convert low 4 bits of hash code
...
static double grad(int64_t hash, double x, double y) {
    const int64_t h = hash & 0x3F;  // Convert low 3 bits of hash code
 ...
 * @return gradient value
 */
static double grad(int64_t hash, double x, double y, double z) {
    int h = hash & 15ull;     // Convert low 4 bits of hash code into 12 simple
...
double SimplexNoise::noise(double x, double y) {
    ...
    const double xs = x + s;
    const double ys = y + s;
    const int64_t i = fastfloor(xs);
    const int64_t j = fastfloor(ys);
